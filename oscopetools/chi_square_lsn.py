...
def chi_square_RFs(responses, LSN_template, num_shuffles=1000):
    ...
    Inputs:
        responses (array with shape [num_trials, num_cells])
        LSN_template (array with shape [num_trials, num_y, num_x])
        num_shuffles (int?)
    ...
    expected_by_pixel = trials_per_pixel.reshape(
        1, num_y, num_x, 2
    ...
    chi_actual, chi_shuffle = chi_square_across_template(
        responses, expected_by_pixel, trial_matrix, num_shuffles
    )
    ...
    # correct p_values for multiple comparisons
    mask_areas = np.sum(masks, axis=1) / 2.0  # on and off pixels
    average_mask_area = mask_areas.mean()
    total_monitor_area = float(num_y * num_x)
    num_comparisons = total_monitor_area / average_mask_area
    p_values = 1.0 - np.power((1.0 - p_values), num_comparisons)
    ...
    for y in range(num_y):
        for x in range(num_x):
            for on_off in range(2):
                frame = np.argwhere(
                    LSN_template[:num_trials, y, x] == on_off_luminance[on_off]
                )[:, 0]
...
def chi_square_across_template(
    responses, expected_by_pixel, trial_matrix, num_shuffles
):
    ...
    observed_by_pixel_actual = calc_observed(
        responses, trials.reshape(1, len(trials)), pixels, num_pixels
    )
    ...
    observed_by_pixel_shuffled = calc_observed(
        responses, shuffled_trials, pixels, num_pixels
    )
...
def rf_mask(p_vals):
    """Return a mask of same shape as p_vals with receptive field of each cell.

    Inputs:
        p_vals (array with shape [no_cells, x, y])
            -- p_values for cell having a receptive field for each pixel

    Returns:
        Mask with same shape as p_vals

    """
    unravelled = p_vals.reshape((p_vals.shape[0], -1))
    unravelled_mask = np.zeros_like(unravelled, dtype=np.bool_)

    # Fill in unravelled_mask for each cell.
    for cell_no in range(unravelled.shape[0]):
        unravelled_mask[cell_no, np.argmin(unravelled[cell_no])] = True

    mask = np.reshape(unravelled_mask, p_vals.shape)
    return mask
