...
def run_analysis():
    ...
    fluorescence = get_wholefield_fluorescence(
        stim_table, im_filetype, im_directory, exp_date, mouse_ID, savepath
    )

    mean_sweep_response, sweep_response = get_mean_sweep_response(
        fluorescence, stim_table
    )

    best_location = plot_sweep_response(
        sweep_response, stim_table, exp_date, mouse_ID, savepath
    )
...
def plot_grid_response(mean_sweep_response, stim_table, exptpath):
    ...
    plt.imshow(
        response_grid,
        vmax=np.max(response_grid),
        vmin=-np.max(response_grid),
        cmap='bwr',
        interpolation='none',
        origin='lower',
    )
...
def get_mean_sweep_response(fluorescence, stim_table):
    ...
    for i in range(num_stim_presentations):
        ...
        mean_sweep_response[i] = np.mean(
            sweep_dff[interlength : (interlength + sweeplength)]
        )
...
def get_wholefield_fluorescence(
    ...
    if os.path.isfile(savepath + exp_date + '_' + mouse_ID + '_wholefield.npy'):
        ...
        if im_filetype == 'nd2':
            ...
            block_bounds.append(
                (
                    np.min(sweep_starts) - 30,
                    np.max(sweep_starts[sweep_starts < 50000]) + 100,
                )
            )
            block_bounds.append(
                (
                    np.min(sweep_starts[sweep_starts > 50000]) - 30,
                    np.max(sweep_starts) + 100,
                )
            )
