...
response_off: mean response, s.e.m., and number of responsive trials for each black square
        ...
        for index, row in self.stim_table.iterrows():
            ...
            sweep_eye.x_pos_deg[index] = self.pupil_pos.x_pos_deg[
                int(row.Start) - 28 : int(row.Start + 35)
            ].values
...
peak dataframe
    ...
    def save_data(self):
        ...
        save_file = os.path.join(
            r'/Users/saskiad/Documents/Data/Openscope_Multiplex/analysis',
            str(self.session_id) + "_lsn_analysis.h5",
        )
        ...
        for nc in range(self.numbercells):
            ...
            for yp in range(8):
                for xp in range(14):
                    ...
                    ax.axvspan(
                        28, 35, ymin=0, ymax=1, facecolor='gray', alpha=0.3
                    )
                    vmax = np.where(
                        np.amax(subset_on.mean()) > vmax,
                        np.amax(subset_on.mean()),
                        vmax,
                    )
                    vmax = np.where(
                        np.amax(subset_off.mean()) > vmax,
                        np.amax(subset_off.mean()),
                        vmax,
                    )
                    vmin = np.where(
                        np.amin(subset_on.mean()) < vmin,
                        np.amin(subset_on.mean()),
                        vmin,
                    )
                    vmin = np.where(
                        np.amin(subset_off.mean()) < vmin,
                        np.amin(subset_off.mean()),
                        vmin,
                    )
                    ax.set_xticks([])
                    ax.set_yticks([])
