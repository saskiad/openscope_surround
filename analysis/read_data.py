...
def get_stimulus_epochs(file_path, session_type):
    if session_type=='size_tuning':
        stim_name_1 = 'drifting_gratings_size'
        ...
        stim_epoch.loc[2] = [0, stim_epoch.Start.iloc[0]-1, 'spontaneous_activity']
        stim_epoch.loc[3] = [stim_epoch.End.iloc[0]+1, stim_epoch.Start.iloc[1]-1, 'spontaneous_activity']
...
def get_epochs(file_path, stim_name_1):
        ...
        stim_epoch.loc[0] = [stim1.Start[0], stim1.End[break1], stim_name_1]
        stim_epoch.loc[1] = [stim1.Start[break1+1], stim1.End.max(), stim_name_1]
        stim_epoch.loc[2] = [stim2.Start[0], stim2.End[break2], 'locally_sparse_noise']
        stim_epoch.loc[3] = [stim2.Start[break2+1], stim2.End.max(), 'locally_sparse_noise']
        stim_epoch.sort_values(by='Start', inplace=True)
        stim_epoch.loc[4] = [0, stim_epoch.Start.iloc[0]-1, 'spontaneous_activity']
        for i in range(1,4):
            stim_epoch.loc[4+i] = [stim_epoch.End.iloc[i-1]+1, stim_epoch.Start.iloc[i]-1, 'spontaneous_activity']
        stim_epoch.sort_values(by='Start', inplace=True)
        stim_epoch.reset_index(inplace=True)
        stim_epoch['Duration'] = stim_epoch.End - stim_epoch.Start
        return stim_epoch
