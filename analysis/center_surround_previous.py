...
#        self.first, self.second = self.cross_validate_response()
        self.metrics, self.OSI, self.DSI, self.ISO, self.ORTHO, self.STRENGTH, self.TUNING, self.CONTEXT, self.DIR = self.get_metrics()
...
#        idx = np.random.choice(range(self.stim_table_spont.Start, self.stim_table_spont.End), 10000)
        ...
        response = np.empty((8,4,self.numbercells, 4)) #center_ori X center/iso/ortho/blank X cells X mean, std, #trials, % significant trials
        ...
        for oi, cori in enumerate(self.orivals):
            for ci, cond in enumerate(self.conditions):
                if cond=='blank':
                    ...
                    subset = mean_sweep_response[(self.stim_table.Center_Ori==cori)&(self.stim_table.condition==cond)&
                                                 (mean_sweep_eye.total<self.eye_thresh)]
                    subset_p = sweep_p_values[(self.stim_table.Center_Ori==cori)&(self.stim_table.condition==cond)&
                                              (mean_sweep_eye.total<self.eye_thresh)]

                response[oi,ci,:,0] = subset.mean(axis=0)
                response[oi,ci,:,1] = subset.std(axis=0)
                response[oi,ci,:,2] = len(subset)
                response[oi,ci,:,3] = subset_p[subset_p<0.05].count().values/float(len(subset))
    ...
    response_second: second half of the response array - same shape.
        ...
        for ci, cond in enumerate(self.conditions):
            if cond=='blank':
                ...
                for oi, cori in enumerate(self.orivals):
                    cell_trials[oi,ci,:,:] = self.mean_sweep_response[(self.stim_table.Center_Ori==cori)&
        ...
        for i in range(n_iter):
            for oi in range(len(self.orivals)):
                for si in range(len(self.conditions)):
                    ...
                    response_first[oi,si,:,i] = np.mean(cell_trials[oi,si,idx_1,:], axis=0)
                    response_second[oi,si,:,i] = np.mean(cell_trials[oi,si,idx_2,:], axis=0)
...
#        cell_index = np.where(np.isfinite(self.dff[:,0]))[0]
        ...
        metrics = pd.DataFrame(columns=('cell_index','center_dir','center_osi','center_dsi','iso','ortho',
...
#            response_second = response_second[:,:,cell_index,:]
            sort = np.where(response_first[:,0,:,ni]==np.nanmax(response_first[:,0,:,ni], axis=(0)))
...
#        sort = np.where(self.response[:,0,:,0] == np.nanmax(self.response[:,0,:,0], axis=0))
        ...
        metrics['center_mean'] = self.response[sort[0][sortind],0,cell_index,0]
        metrics['center_std'] = self.response[sort[0][sortind],0,cell_index,1]
        metrics['center_percent_trials'] = self.response[sort[0][sortind],0,cell_index,3]
        ...
        metrics['ortho_mean'] = self.response[sort[0][sortind],2,cell_index,0]
    ...
    def save_data(self):
        ...
        save_file = os.path.join(r'/Users/saskiad/Documents/Data/Openscope_Multiplex_trim/analysis', str(self.session_id)+"_cs_analysis.h5")
