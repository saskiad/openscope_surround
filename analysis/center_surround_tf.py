...
response_std: std of response to each stimulus condition
        ...
        response = np.empty(
            (8, 2, 4, self.numbercells, 4)
        )  # center_ori X TF x center/iso/ortho/blank X cells X mean, std, #trials, % significant trials
...
metrics dataframe
        ...
        for ni in range(n_iter):
            ...
            sort = np.where(
                response_first[:, :, 0, :, ni]
                == np.nanmax(response_first[:, :, 0, :, ni], axis=(0, 1))
            )
            ...
            OSI.loc[ni] = self.get_osi(
                response_second[:, inds[1], 0, inds[2], ni]
            )
        ...
        sort = np.where(
            ...
            == np.nanmax(self.response[:, :, 0, cell_index, 0], axis=(0, 1))
        )
        ...
        metrics['center_mean'] = self.response[
            pref_ori, pref_tf, 0, cell_index, 0
        ]
        metrics['center_std'] = self.response[
            pref_ori, pref_tf, 0, cell_index, 1
        ]
        metrics['center_percent_trials'] = self.response[
            pref_ori, pref_tf, 0, cell_index, 3
        ]
        ...
        metrics['ortho_mean'] = self.response[
            pref_ori, pref_tf, 2, cell_index, 0
        ]
        metrics['ortho_std'] = self.response[
            pref_ori, pref_tf, 2, cell_index, 1
        ]
