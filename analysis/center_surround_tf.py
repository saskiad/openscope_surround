...
class CenterSurround:
    def __init__(self, expt_path, eye_thresh, cre, area, depth):
        ...
        self.stim_table.loc[self.stim_table.Center_Ori==self.stim_table.Surround_Ori, 'condition'] = 'iso'
        self.stim_table.loc[np.isfinite(self.stim_table.Center_Ori)&np.isnan(self.stim_table.Surround_Ori), 'condition'] = 'center'
        self.stim_table.loc[np.isnan(self.stim_table.Center_Ori)&np.isnan(self.stim_table.Surround_Ori), 'condition'] = 'blank'
        self.stim_table.loc[np.isnan(self.stim_table.Center_Ori)&np.isfinite(self.stim_table.Surround_Ori), 'condition'] = 'surround'
...
#        self.first, self.second = self.cross_validate_response(n_trials=int(self.response[:,:,:,:,2].min()))
        self.metrics, self.OSI, self.DSI, self.ISO, self.ORTHO, self.STRENGTH, self.TUNING, self.CONTEXT = self.get_metrics()
...
#                sweep_response[str(nc)][index] = ((temp/np.mean(temp[:30]))-1)
            sweep_eye.x_pos_deg[index] = self.pupil_pos.x_pos_deg[int(row.Start)-30:int(row.Start+90)].values
            sweep_eye.y_pos_deg[index] = self.pupil_pos.y_pos_deg[int(row.Start)-30:int(row.Start+90)].values
...
#        idx = np.random.choice(range(self.stim_table_spont.Start, self.stim_table_spont.End), 10000)
        idx = np.random.choice(range(int(self.stim_table_spont.Start), int(self.stim_table_spont.End)), 10000)
        for i in range(60):
        ...
        response = np.empty((8, 2, 4, self.numbercells, 4)) #center_ori X TF x center/iso/ortho/blank X cells X mean, std, #trials, % significant trials

        for oi, cori in enumerate(self.orivals):
            for ti, tf in enumerate(self.tfvals):
                for ci, cond in enumerate(self.conditions):
                    if cond=='blank':
                        subset = mean_sweep_response[(self.stim_table.condition==cond)&(mean_sweep_eye.total<self.eye_thresh)]
                        subset_p = sweep_p_values[(self.stim_table.condition==cond)&(mean_sweep_eye.total<self.eye_thresh)]
                    else:
                        subset = mean_sweep_response[(self.stim_table.Center_Ori==cori)&(self.stim_table.TF==tf)&
                                                     (self.stim_table.condition==cond)&(mean_sweep_eye.total<self.eye_thresh)]
                        subset_p = sweep_p_values[(self.stim_table.Center_Ori==cori)&(self.stim_table.TF==tf)&
                                                  (self.stim_table.condition==cond)&(mean_sweep_eye.total<self.eye_thresh)]

                    response[oi,ti,ci,:,0] = subset.mean(axis=0)
                    response[oi,ti,ci,:,1] = subset.std(axis=0)
                    response[oi,ti,ci,:,2] = len(subset)
                    response[oi,ti,ci,:,3] = subset_p[subset_p<0.05].count().values/float(len(subset))
    ...
    response_second: second half of the response array - same shape.
        '''
        cell_trials = np.empty((8, 2, 4, n_trials, self.numbercells))
        cell_trials[:] = np.NaN

        response_first = np.empty((8, 2, 4, self.numbercells, n_iter))
        response_second = np.empty((8, 2, 4, self.numbercells, n_iter))
        ...
        for ci, cond in enumerate(self.conditions):
            if cond=='blank':
                cell_trials[:,:,ci,:,:] = self.mean_sweep_response[(self.stim_table.condition==cond)&
                           (self.mean_sweep_eye.total<self.eye_thresh)].values[:n_trials]
            else:
                for oi, cori in enumerate(self.orivals):
                    for ti, tf in enumerate(self.tfvals):
                        cell_trials[oi,ti,ci,:,:] = self.mean_sweep_response[(self.stim_table.Center_Ori==cori)&(self.stim_table.TF==tf)&
                                   (self.stim_table.condition==cond)&(self.mean_sweep_eye.total<self.eye_thresh)].values[:n_trials]
...
metrics dataframe
        ...
        metrics = pd.DataFrame(columns=('cell_index','center_dir','center_tf','center_osi','center_dsi','iso','ortho',
                                        'suppression_strength','suppression_tuning','cmi','dir_percent'), index=cell_index)
        ...
        ORTHO = pd.DataFrame(columns=cell_index.astype(str), index=range(n_iter))
        STRENGTH = pd.DataFrame(columns=cell_index.astype(str), index=range(n_iter))
        TUNING = pd.DataFrame(columns=cell_index.astype(str), index=range(n_iter))
        CONTEXT = pd.DataFrame(columns=cell_index.astype(str), index=range(n_iter))
...
#            response_second = response_second[:,:,:,cell_index,:]
            sort = np.where(response_first[:,:,0,:,ni]==np.nanmax(response_first[:,:,0,:,ni], axis=(0,1)))
            #TODO: this is where the TF is going to add issues...
            sortind = np.argsort(sort[2])
            pref_ori = sort[0][sortind]
            pref_tf = sort[1][sortind]
            cell_index = sort[2][sortind]
            inds = np.vstack((pref_ori, pref_tf, cell_index))

            DIR.loc[ni] = pref_ori

            #osi
            OSI.loc[ni] = self.get_osi(response_second[:, inds[1], 0, inds[2], ni])

            #dsi
            null_ori= np.mod(pref_ori+4, 8)
            pref = response_second[inds[0], inds[1], 0, inds[2], ni]
            null = response_second[null_ori, inds[1], 0, inds[2],  ni]
            null = np.where(null>0, null, 0)
            DSI.loc[ni] = (pref-null)/(pref+null)

            center = response_second[inds[0], inds[1], 0, inds[2], ni]
            iso = response_second[inds[0], inds[1], 1, inds[2], ni]
            ortho = response_second[inds[0], inds[1], 2, inds[2], ni]
        ...
        cell_index = np.array(range(self.numbercells))
        sort = np.where(self.response[:,:,0,cell_index,0] == np.nanmax(self.response[:,:,0,cell_index,0], axis=(0,1)))
#        sort = np.where(self.response[:,0,:,0] == np.nanmax(self.response[:,0,:,0], axis=0))
        sortind = np.argsort(sort[2])
        pref_ori = sort[0][sortind]
        pref_tf = sort[1][sortind]
        cell_index = sort[2][sortind]
        metrics['center_dir'] = pref_ori
        metrics['center_tf'] = pref_tf
        metrics['center_mean'] = self.response[pref_ori,pref_tf,0,cell_index,0]
        metrics['center_std'] = self.response[pref_ori,pref_tf,0,cell_index,1]
        metrics['center_percent_trials'] = self.response[pref_ori, pref_tf,0,cell_index,3]
        metrics['blank_mean'] = self.response[0,0,3,cell_index,0]
        metrics['blank_std'] = self.response[0,0,3,cell_index,1]
        metrics['iso_mean'] = self.response[pref_ori,pref_tf,1,cell_index,0]
        metrics['iso_std'] = self.response[pref_ori,pref_tf,1,cell_index,1]
        metrics['ortho_mean'] = self.response[pref_ori,pref_tf,2,cell_index,0]
        metrics['ortho_std'] = self.response[pref_ori,pref_tf,2,cell_index,1]
    ...
    def save_data(self):
        ...
        save_file = os.path.join(r'/Users/saskiad/Documents/Data/Openscope_Multiplex_trim/analysis/tf', str(self.session_id)+"_cs_analysis.h5")
...
#    expt_path = r'/Users/saskiad/Documents/Data/Openscope_Multiplex_trim/Center_Surround_1006636506_data.h5'
#    eye_thresh = 10
#    cre = 'test'
#    area = 'area test'
#    depth = '33'
...
#            expt_path = r'/Users/saskiad/Dropbox/Openscope Multiplex/Center Surround/Center_Surround_'+str(int(row.Center_Surround_Expt_ID))+'_data.h5'
            expt_path = r'/Users/saskiad/Documents/Data/Openscope_Multiplex_trim/Center_Surround_'+str(int(row.Center_Surround_Expt_ID))+'_data.h5'
            eye_thresh = 10
            try:
                cs = CenterSurround(expt_path=expt_path, eye_thresh=eye_thresh, cre=cre, area=area, depth=depth)
                if count==1:
