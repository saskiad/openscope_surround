        ...
        for oi, ori in enumerate(self.orivals):
            for ti, tf in enumerate(self.tfvals):
                for si, size in enumerate(self.sizevals):
                    subset = mean_sweep_response[(self.stim_table.Ori==ori)&(self.stim_table.TF==tf)&
...
#            response_second = response_second[:,:,:,cell_index,:]
            sort = np.where(response_first[:,:,:,:,ni]==np.nanmax(response_first[:,:,:,:,ni], axis=(0,1,2)))
...
#            print(len(pref_ori))
        ...
        cell_index = np.array(range(self.numbercells))
        sort = np.where(self.response[:,:,:,cell_index,0] == np.nanmax(self.response[:,:,:,cell_index,0], axis=(0,1,2)))
        sortind = np.argsort(sort[3])
        pref_ori = sort[0][sortind]
        pref_tf = sort[1][sortind]
        pref_size = sort[2][sortind]
        ...
        metrics['peak_mean'] = self.response[pref_ori,pref_tf,pref_size,cell_index,0]
        metrics['peak_std'] = self.response[pref_ori,pref_tf,pref_size,cell_index,1]
    ...
    def save_data(self):
        ...
        save_file = os.path.join(r'/Users/saskiad/Documents/Data/Openscope_Multiplex_trim/analysis/tf', str(self.session_id)+"_st_analysis.h5")
...
#    szt = SizeTuning(expt_path=expt_path, eye_thresh=eye_thresh, cre=cre, area=area, depth=depth)
    ...
    for index, row in subset.iterrows():
        if np.isfinite(row.Size_Tuning_Expt_ID):
            ...
            area = row.Area
            depth = row.Depth
            expt_path = r'/Users/saskiad/Documents/Data/Openscope_Multiplex_trim/Size_Tuning_'+str(int(row.Size_Tuning_Expt_ID))+'_data.h5'
            eye_thresh = 10
            try:
                szt = SizeTuning(expt_path=expt_path, eye_thresh=eye_thresh, cre=cre, area=area, depth=depth)
                if count==1:
