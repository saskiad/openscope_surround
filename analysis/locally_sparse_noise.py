...
class LocallySparseNoise:
    def __init__(self, expt_path):
        ...
        self.session_id = self.expt_path.split('/')[-1] #this might need to be modified for where the data is for you.
        ...
        #run analysis
        self.sweep_response, self.mean_sweep_response, self.sweep_p_values, self.response_on, self.response_off = self.get_stimulus_response(self.LSN)
...
#                response_off[xp,yp,:,2] = subset_off_p[subset_off_p<0.05].count().values/float(len(subset_off_p))
        return sweep_response, mean_sweep_response, sweep_p_values, response_on, response_off
