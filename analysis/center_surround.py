...
#        cell_index = np.where(np.isfinite(self.dff[:,0]))[0]
        ...
        metrics = pd.DataFrame(columns=('center_dir','center_osi','center_dsi','iso','ortho', 
                                        'suppression_strength','suppression_tuning','cmi'), index=cell_index)
...
#        cell_index = np.where(np.isfinite(self.dff[:,0]))[0]
        cell_index = np.array(range(self.numbercells))
        sort = np.where(self.response[:,0,cell_index,0] == np.nanmax(self.response[:,0,cell_index,0], axis=0))
        sortind = np.argsort(sort[1])
    ...
    def save_data(self):
        ...
        save_file = os.path.join(r'/Users/saskiad/Documents/Data/Openscope_Multiplex/analysis_py3', str(self.session_id)+"_cs_analysis.h5")
