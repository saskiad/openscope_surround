...
>>>>>>> Stashed changes
        ...
        idx = np.random.choice(
            list(range(self.stim_table_sp.start[0], self.stim_table_sp.end[0])),
            10000,
        )
        for i in range(10):
        ...
        for xp in range(x_shape):
            for yp in range(y_shape):
                ...
                subset_on = mean_sweep_events[
                    self.stim_table.frame.isin(on_frame)
                ]
                subset_on_p = sweep_p_values[
                    self.stim_table.frame.isin(on_frame)
                ]
                subset_off = mean_sweep_events[
                    self.stim_table.frame.isin(off_frame)
                ]
                subset_off_p = sweep_p_values[
                    self.stim_table.frame.isin(off_frame)
                ]
                response_events_on[xp, yp, :, 0] = subset_on.mean(axis=0)
                response_events_on[xp, yp, :, 1] = subset_on.std(
                    axis=0
                ) / np.sqrt(len(subset_on))
                response_events_on[xp, yp, :, 2] = subset_on_p[
                    subset_on_p < 0.05
                ].count().values / float(len(subset_on_p))
                response_events_off[xp, yp, :, 0] = subset_off.mean(axis=0)
                response_events_off[xp, yp, :, 1] = subset_off.std(
                    axis=0
                ) / np.sqrt(len(subset_off))
                response_events_off[xp, yp, :, 2] = subset_off_p[
                    subset_off_p < 0.05
                ].count().values / float(len(subset_off_p))
        return (
            sweep_events,
            mean_sweep_events,
            sweep_p_values,
...
peak dataframe
        ...
        if lsn_name == 'locally_sparse_noise':
            peak = pd.DataFrame(
                columns=('cell_specimen_id', 'rf_on', 'rf_off'),
                index=list(range(self.numbercells)),
            )
            peak['rf_on'] = False
            peak['rf_off'] = False
            ...
            peak['rf_off_8deg'] = False
            peak.cell_specimen_id = self.specimen_ids
            on_rfs = np.where(self.response_events_on_4deg[:, :, :, 2] > 0.25)[
                2
            ]
            off_rfs = np.where(
                self.response_events_off_4deg[:, :, :, 2] > 0.25
            )[2]
            peak.rf_on_4deg.loc[on_rfs] = True
            peak.rf_off_4deg.loc[off_rfs] = True
            on_rfs = np.where(self.response_events_on_8deg[:, :, :, 2] > 0.25)[
                2
            ]
            off_rfs = np.where(
                self.response_events_off_8deg[:, :, :, 2] > 0.25
            )[2]
            peak.rf_on_8deg.loc[on_rfs] = True
            peak.rf_off_8deg.loc[off_rfs] = True
        return peak

    def save_data(self, lsn_name):
        save_file = os.path.join(
            self.save_path, str(self.session_id) + "_lsn_events_analysis.h5"
        )
        ...
        if lsn_name == 'locally_sparse_noise':
            ...
            dset = f.create_dataset(
                'response_events_on', data=self.response_events_on
            )
            dset1 = f.create_dataset(
                'response_events_off', data=self.response_events_off
            )
            f.close()
        else:
            store = pd.HDFStore(save_file)
            store['sweep_events_4deg'] = self.sweep_events_4deg
            store['mean_sweep_events_4deg'] = self.mean_sweep_events_4deg
            store['sweep_p_values_4deg'] = self.sweep_p_values_4deg
            store['running_speed_4deg'] = self.running_speed_4deg
            store['sweep_events_8deg'] = self.sweep_events_8deg
            store['mean_sweep_events_8deg'] = self.mean_sweep_events_8deg
            store['sweep_p_values_8deg'] = self.sweep_p_values_8deg
            store['running_speed_8deg'] = self.running_speed_8deg
            store['peak'] = self.peak
            store.close()
            f = h5py.File(save_file, 'r+')
            dset = f.create_dataset(
                'response_events_on_4deg', data=self.response_events_on_4deg
            )
            dset1 = f.create_dataset(
                'response_events_off_4deg', data=self.response_events_off_4deg
            )
            dset = f.create_dataset(
                'response_events_on_8deg', data=self.response_events_on_8deg
            )
            dset1 = f.create_dataset(
                'response_events_off_8deg', data=self.response_events_off_8deg
            )
            f.close()
