...
peak dataframe
        ...
        if lsn_name=='locally_sparse_noise':
            ...
            peak.cell_specimen_id = self.specimen_ids
            on_rfs = np.where(self.response_events_on_4deg[:,:,:,2]>0.25)[2]
            off_rfs = np.where(self.response_events_off_4deg[:,:,:,2]>0.25)[2]
            peak.rf_on_4deg.loc[on_rfs] = True 
            peak.rf_off_4deg.loc[off_rfs] = True 
            on_rfs = np.where(self.response_events_on_8deg[:,:,:,2]>0.25)[2]
            off_rfs = np.where(self.response_events_off_8deg[:,:,:,2]>0.25)[2]
    ...
    def save_data(self, lsn_name):
        ...
        if lsn_name=='locally_sparse_noise':
            ...
            store.close()
            f = h5py.File(save_file, 'r+')
            dset = f.create_dataset('response_events_on_4deg', data=self.response_events_on_4deg)
            dset1 = f.create_dataset('response_events_off_4deg', data=self.response_events_off_4deg)
            dset = f.create_dataset('response_events_on_8deg', data=self.response_events_on_8deg)
            dset1 = f.create_dataset('response_events_off_8deg', data=self.response_events_off_8deg)
            f.close()
