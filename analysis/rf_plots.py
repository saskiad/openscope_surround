    ...
    for session in os.listdir(os.path.join(DATA_PATH, specimen)):
        ...
        with util.gagProcess():
            try:
                ...
                warnings.warn(
                    'IOError caught trying to make stim_table from {}'.format(tmp_path)
                )
...
for specimen in list(stim_stuff.keys()):
    for session in list(stim_stuff[specimen].keys()):
        ...
        plt.suptitle('RF summary: {} {} {}um depth'.format(
            session,
            experiments_df.loc[experiments_df['ophys_sesssion'] == session[-9:], 'genotype_name'].values[0],
            experiments_df.loc[experiments_df['ophys_sesssion'] == session[-9:], 'imaging_depth'].values[0]
        ))
